...
/**
 * REST controller for managing {@link com.example.gestionAchat.domain.Facture}.
 */
@RestController
@RequestMapping("/api")
...
//        }
    ...
    public List<Facture> getAllFactures(@RequestParam(required = false) String filter) {
        if ("processus-is-null".equals(filter)) {
            log.debug("REST request to get all Factures where processus is null");
            return StreamSupport
                .stream(factureRepository.findAll().spliterator(), false)
                .filter(facture -> facture.getProcessus() == null)
