...
/**
 * REST controller for managing {@link com.example.gestionAchat.domain.Article}.
 */
...
//spring.application.name
     ...
     */
    @GetMapping("/articles/demande/{id}")
    public List<Article> getDemandeArticle(@PathVariable Long id){

        log.debug("REST request to get Article : {}", id);
        List<Article> article = articleRepository.findAllByDemandeAchat_Id(id);
        return article;
    }
...
//        }
    ...
    @GetMapping("/getAllDemandearticles")
    public List<Article> getAllDemandearticles(@RequestParam("demandeId") Long demandeId) {
        log.debug("REST request to get Article : {}", demandeId);


        String querySelect = "SELECT * FROM article where article.id IN (SELECT demande_achat_article.article_id FROM demande_achat_article WHERE demande_achat_article.demande_achat_id = "+demandeId+") ";

        Query q1 = em.createNativeQuery(querySelect, Article.class);

        List<Article> articles = q1.getResultList();
        return articles;
    }
